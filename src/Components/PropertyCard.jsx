    ...
    <TouchableOpacity
      ...
      <TouchableOpacity
        ...
        <Image
          ...
          style={{
            width: 20,
            height: 20,
            tintColor: item?.is_wishlist == 1 ? COLOR?.primary : COLOR.grey,
          }}
        />
      </TouchableOpacity>
      ...
      <Text style={styles.propertyLocation}>{item?.preferred_tenant_type || JSON.parse(item?.property?.preferred_tenant_type)} </Text>
        ...
        {item?.property?.price != null && item?.property?.price !== undefined
          ...
          {(item?.status || item?.property?.status) == 1 && <Text style={styles.TagStyle}>Featured</Text>}
          </View>
...
const styles = StyleSheet.create({
  ...
  TagStyle:{paddingVertical:2,backgroundColor:COLOR.primary,width:'38%',color:'white',fontWeight:500,textAlign:'center',borderRadius:2,fontSize:12}
});
